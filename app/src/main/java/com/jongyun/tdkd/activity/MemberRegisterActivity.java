...
public class MemberRegisterActivity extends Activity {
                ...
                public void onResponse(Call<String> call, Response<String> response) {
                    if(response.isSuccessful()) {
                        if ("DUP".equals(response.body())) {
                            ...
                            Toast.makeText(getApplicationContext(), "다시 한번 시도해 주십시오.", Toast.LENGTH_LONG).show();
                        }
                    }
                }
    ...
    private boolean checkRegisterValidation() {
        ...
        if(id.getText().toString().length() > 12) {
            Toast.makeText(getApplicationContext(), "ID의 최대 길이는 12자리 이하입니다.", Toast.LENGTH_LONG).show();
            return false;
        }

        if(password.getText().toString().trim().isEmpty()) {
            Toast.makeText(getApplicationContext(), "사용할 비밀번호를 입력해 주세요.", Toast.LENGTH_LONG).show();
            return false;
        }

        if(password.getText().toString().length() < 6) {
            Toast.makeText(getApplicationContext(), "비밀번호는 6자리 이상을 사용해 주십시오.", Toast.LENGTH_LONG).show();
            return false;
        }

        if(password.getText().toString().length() > 20) {
            Toast.makeText(getApplicationContext(), "비밀번호는 20자리 이하를 사용해 주십시오.", Toast.LENGTH_LONG).show();
            return false;
        }

        if(name.getText().toString().isEmpty()) {
            Toast.makeText(getApplicationContext(), "이름을 입력해 주십시오.", Toast.LENGTH_LONG).show();
            return false;
        }

        if(name.getText().toString().length() > 10) {
            Toast.makeText(getApplicationContext(), "이름은 10자리 이하를 사용해 주십시오.", Toast.LENGTH_LONG).show();
            return false;
        }

        if(!email.getText().toString().matches("^[_0-9a-zA-Z-]+@[0-9a-zA-Z-]+(.[_0-9a-zA-Z-]+)*$")) {
            Toast.makeText(getApplicationContext(), "정상적인 이메일 형식이 아닙니다.", Toast.LENGTH_LONG).show();
            return false;
        }

        if(address.getText().toString().isEmpty()) {
            Toast.makeText(getApplicationContext(), "주소를 입력해 주십시오.", Toast.LENGTH_LONG).show();
            return false;
        }

        if(address.getText().toString().length() > 50) {
            Toast.makeText(getApplicationContext(), "주소는 50자리 이하를 사용해 주십시오.", Toast.LENGTH_LONG).show();
            return false;
        }

        if(phoneNumber.getText().toString().isEmpty()) {
            Toast.makeText(getApplicationContext(), "휴대폰 번호를 입력해 주십시오.", Toast.LENGTH_LONG).show();
            return false;
        }

        if(!phoneNumber.getText().toString().matches("^01(?:0|1[6-9])-(?:\\d{3}|\\d{4})-\\d{4}$")) {
            Toast.makeText(getApplicationContext(), "정상적인 휴대폰 번호 형식이 아닙니다.", Toast.LENGTH_LONG).show();
            return false;
        }
